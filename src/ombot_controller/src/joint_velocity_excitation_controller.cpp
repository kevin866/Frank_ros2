#include "joint_velocity_excitation_controller/joint_velocity_excitation_controller.hpp"
...
JointVelocityExcitationController::command_interface_configuration() const
{
...
controller_interface::InterfaceConfiguration
JointVelocityExcitationController::state_interface_configuration() const
{
...
JointVelocityExcitationController::on_init()
{
  try {
    ...
    RCLCPP_ERROR(get_node()->get_logger(),
                 "JointVelocityExcitationController on_init exception: %s", e.what());
...
controller_interface::CallbackReturn
JointVelocityExcitationController::on_configure(const rclcpp_lifecycle::State &)
{
  ...
  if (joint_names_.empty()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "JointVelocityExcitationController: 'joints' param must be non-empty.");
    return controller_interface::CallbackReturn::ERROR;
  }

  const size_t N = joint_names_.size();

  if (!velocity_limits_.empty() && velocity_limits_.size() != N) {
    RCLCPP_WARN(get_node()->get_logger(),
                "velocity_limits size (%zu) != joints (%zu). Ignoring limits.",
                velocity_limits_.size(), N);
...
controller_interface::CallbackReturn
JointVelocityExcitationController::on_activate(const rclcpp_lifecycle::State &)
{
  if (command_interfaces_.empty()) {
    RCLCPP_ERROR(get_node()->get_logger(),
                 "JointVelocityExcitationController: no command interfaces on activate.");
    return controller_interface::CallbackReturn::ERROR;
  }
  ...
  std::fill(qdot_cmd_.begin(), qdot_cmd_.end(), 0.0);
  active_ = true;

  RCLCPP_INFO(get_node()->get_logger(), "JointVelocityExcitationController activated.");
  return controller_interface::CallbackReturn::SUCCESS;
}

controller_interface::CallbackReturn
JointVelocityExcitationController::on_deactivate(const rclcpp_lifecycle::State &)
{
  active_ = false;
  for (auto & ci : command_interfaces_) {
    ci.set_value(0.0);
  }
  RCLCPP_INFO(get_node()->get_logger(), "JointVelocityExcitationController deactivated.");
  return controller_interface::CallbackReturn::SUCCESS;
}

void JointVelocityExcitationController::clamp_velocity(std::vector<double> & qdot) const
{
  if (velocity_limits_.empty()) return;
  for (size_t i = 0; i < qdot.size(); ++i) {
    const double lim = std::abs(velocity_limits_[i]);
    qdot[i] = std::clamp(qdot[i], -lim, lim);
  }
}


void JointVelocityExcitationController::init_prbs()
  {
    const size_t N = joint_names_.size();
...
// void JointVelocityExcitationController::update_prbs(const rclcpp::Time & now)
// {
...
//   const size_t N = joint_names_.size();
//   for (size_t i = 0; i < N; ++i) {
//     qdot_cmd_[i] = static_cast<double>(prbs_state_[i]) * amplitudes_[i];
//   }
// }
...
void JointVelocityExcitationController::compute_sine(double t)
{
  const double w = 2.0 * M_PI * sine_freq_;  // rad/s
  const size_t N = joint_names_.size();
  const double s = std::sin(w * t);
  for (size_t i = 0; i < N; ++i) {
    qdot_cmd_[i] = amplitudes_[i] * s;
  }
}

void JointVelocityExcitationController::compute_chirp(double t)
{
...
JointVelocityExcitationController::update(const rclcpp::Time & time,
                                  const rclcpp::Duration & /*period*/)
{
  ...
  if (command_interfaces_.size() != N) {
    RCLCPP_ERROR_THROTTLE(
      ...
      "JointVelocityExcitationController: cmd interfaces (%zu) != joints (%zu)",
  ...
  clamp_velocity(qdot_cmd_);

  // Write to hardware
  for (size_t i = 0; i < N; ++i) {
    command_interfaces_[i].set_value(qdot_cmd_[i]);
  }

  if (qdot_pub_ && qdot_pub_->trylock()) {
    auto & msg = qdot_pub_->msg_;
    msg.data = qdot_cmd_;   // copy vector<double> -> data
    qdot_pub_->unlockAndPublish();
  }
...
PLUGINLIB_EXPORT_CLASS(
  ombot_controller::JointVelocityExcitationController,
